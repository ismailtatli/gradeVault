...
async function renderDashboard(app) {
  app.innerHTML = `<div class="stats-grid" id="stats"></div><div id="recent-courses"></div>`;
  ...
  const passing = graded.filter(d => d.average >= 60).length;
  ...
  document.getElementById('stats').innerHTML = `
    ...
    <div class="stat-card">
      ...
      <div class="stat-sub">out of ${graded.length} graded</div>
    </div>
    <div class="stat-card">
      <div class="stat-label">Total Credits</div>
      <div class="stat-value">${gpaData.totalCredits || 0}</div>
      <div class="stat-sub">Credits</div>
    </div>
  `;

  const recentDiv = document.getElementById('recent-courses');
  recentDiv.innerHTML = `<div class="page-header"><h2 class="page-title">My Courses</h2></div>`;
  if (courses.length === 0) {
    recentDiv.innerHTML += `<div class="empty-state"><div class="empty-state-icon">📚</div><p>No courses yet. Go to Courses tab to add one.</p></div>`;
    return;
  }
...
async function loadCourses(search = '') {
  ...
  if (courses.length === 0) {
    list.innerHTML = `<div class="empty-state" style="grid-column:1/-1"><div class="empty-state-icon">🔍</div><p>No courses found.</p></div>`;
    return;
  }
...
function courseCardHTML(c) {
  const avg = c.average;
  let badgeClass = '';
  if (avg != null) badgeClass = avg >= 60 ? 'pass' : 'fail';
  return `
    <div class="course-card" data-id="${c.id}">
      <div class="course-card-header">
        ...
        <div class="grade-badge ${badgeClass}">${avg != null ? c.letterGrade : '—'}</div>
      </div>
      ${avg != null
        ? `<div class="progress-bar"><div class="progress-fill" style="width:${avg}%"></div></div>
           <div style="font-size:0.75rem;color:var(--text-muted);margin-top:0.3rem">${avg.toFixed(1)} / 100</div>`
...
function bindCourseCardEvents(container) {
  ...
  container.querySelectorAll('.edit-course-btn').forEach(btn => {
    btn.addEventListener('click', e => { e.stopPropagation(); openCourseForm(Number(btn.dataset.id)); });
  });
  container.querySelectorAll('.delete-course-btn').forEach(btn => {
    btn.addEventListener('click', e => { e.stopPropagation(); deleteCourse(Number(btn.dataset.id)); });
  });
}
  ...
  openModal(isEdit ? 'Edit Course' : 'Add New Course', `
    ...
    <div class="form-row">
      ...
      <div class="form-group">
        ...
        <input class="form-control" id="f-code" placeholder="e.g. CS201" />
      </div>
    </div>
    <div class="form-row">
      <div class="form-group">
      ...
      <div class="form-group">
        ...
        <input class="form-control" id="f-semester" placeholder="e.g. Fall 2025" />
      </div>
    </div>
    <div class="form-group">
...
async function deleteCourse(id) {
  ...
  if (res.success) { showToast('Course deleted.', 'error'); loadCourses(''); }
}
    ...
    <div class="card">
      <div class="table-wrapper">
        <table>
          <thead>
            <tr>
              <th>Title</th><th>Type</th><th>Weight</th>
              <th>Grade</th><th>Due Date</th><th>Actions</th>
            </tr>
          </thead>
...
function renderAssignmentRows(assignments, courseId) {
  ...
  if (assignments.length === 0) {
    tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;color:var(--text-muted);padding:2rem">No assignments yet.</td></tr>`;
    return;
  }
...
async function deleteAssignment(courseId, id) {
  ...
  const res = await apiFetch(`/courses/${courseId}/assignments/${id}`, { method: 'DELETE' });
  if (res.success) { showToast('Assignment deleted.', 'error'); renderView('course-detail'); }
}
...
async function renderGPA(app) {
  ...
  app.innerHTML = `
    <div class="page-header"><h2 class="page-title">GPA Calculator</h2></div>
...
async function loadGPA(semester = '') {
  ...
  if (details.length === 0) {
    detailDiv.innerHTML = `<div class="empty-state"><div class="empty-state-icon">📊</div><p>No courses found.</p></div>`;
    return;
  }
  detailDiv.innerHTML = `
    <div class="table-wrapper">
      <table>
        <thead>
          <tr>
            ...
            <th>Average</th><th>Letter</th><th>GPA Points</th>
          </tr>
        </thead>
        <tbody>
          ${details.map(d => `
